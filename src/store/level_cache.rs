...
use sds::service::storage::download::{sds_is_enable, reader_from_env};
...
impl MixFile {
    ...
    fn sds_open(path: &str, len: usize) -> std::io::Result<MixFile> {
        let r = reader_from_env(path).unwrap().read_last_bytes(len)?;
        trace!("read sds open {} {}", path, len);
        trace!("sds data {} {}", &r.1[0], &r.1[len - 1]);
        return Ok(MixFile {
            file: None,
...
impl<E: Element, R: Read + Send + Sync> Store<E> for LevelCacheStore<E, R> {
    ...
    fn new_from_disk_v2(
        ...
        let data_path = StoreConfig::data_path(&config.path, &config.id);
        let file = if post && sds_is_enable() {
            MixFile::sds_open(data_path.to_str().unwrap(), E::byte_len())?
        } else {
